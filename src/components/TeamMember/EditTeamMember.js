...
class EditTeamMember extends Component {
    ...
    componentDidMount()
    {
        database.ref("teammembers").child(`${this.props.match.params.id}`).on('value', (snapshot) => {
            const {firstName, lastName, phoneNumber, bio, profileImage, linkedInUrl} = snapshot.val();
            this.setState({
                firstName,
                lastName,
                phoneNumber,
    ...
    handleSubmit(e)
    {
        ...
        database.ref("teammembers").child(`${this.props.match.params.id}`).update(teamMemberItem);
